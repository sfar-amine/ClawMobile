...
LOG=$STATE/adb-supervisor.log
LOCK=$STATE/adb-supervisor.lock
CHILD=$HOME_DIR/ClawMobile/installer/termux-lite/adb-recovery-watchdog.sh
mkdir -p "$STATE"
exec 9>"$LOCK"
flock -n 9 || exit 0
log(){ printf '%s component=adb-supervisor %s\n' "$(date -Iseconds)" "$*" >>"$LOG"; }
backoff=2
log 'event=start result=ok'
while :; do
  started=$(date +%s)
  log 'event=child_start result=attempt'
  "$CHILD" >>"$STATE/adb-recovery.stderr.log" 2>&1
  rc=$?
