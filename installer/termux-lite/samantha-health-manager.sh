...
check_adb(){ if adb_ok; then prev=$(get adb status unknown); healthy adb; [ "$prev" = healthy ] || "$ROOT/chat-continuity-restore.sh" >/dev/null 2>&1 || true; return; fi; due adb || return; log 'service=adb action=fast_reconnect'; timeout 4 adb connect 127.0.0.1:5556 >/dev/null 2>&1||true; sleep 1; adb_ok && healthy adb || fail adb; }
